...
def build_run_log_payload(
    ...
    return {
        ...
        "windows_user": _user_name(),
        "computer_name": platform.node() or "",
        "source": entry.source or "",
        "error": entry.error or "",
        "failed_step": entry.failed_step if entry.failed_step is not None else 0,
        "attempts": entry.attempts,
        "retry_count": entry.retry_count,
        "fallback_executed": "True" if entry.fallback_executed else "False",
        "evidence_path": entry.evidence_path or "",
